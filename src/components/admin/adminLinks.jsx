...
const AdminLinks = ({ mobile }) => {
  ...
  const isActive = (path, exact = false) => {
    ...
    return (
      ...
      <li data-bs-dismiss={mobile && "offcanvas"} className="nav-item w-100 py-2">
        <Link to="/dashboard/orders" className={`py-3 customlinks nav-link ${isActive("/dashboard/orders") ? "active" : ""}`}>
          <i className="fa fa-shopping-bag me-2"></i> <span className="ms-1 text-light">Orders</span>
        </Link>
      </li>
        </>
    )
